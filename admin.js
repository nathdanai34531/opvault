...
async function autoFetchCardData() {
    ...
    status.textContent = "กำลังค้นหาใน Master Database...";

    try {
        let catalog = {};
        try {
            // Load local database file generated by python scraper
            const response = await fetch('card_catalog.json');
            if (response.ok) {
                catalog = await response.json();
            } else {
                throw new Error("Cannot load catalog file");
            }
        } catch (e) {
            throw new Error('ไม่พบไฟล์ฐานข้อมูล card_catalog.json ในระบบ โปรดรันสคริปต์ดึงข้อมูลก่อน');
        }
        
        // Exact match or partial match (e.g. searching OP01-001 or OP01-001_p1)
        // Usually, the admin types exactly "OP16-001". The catalog might have "OP16-001".
        let cardData = catalog[cardId];
        
        // If not found, try to find a parallel art if they typed it, or just first match
        if (!cardData) {
            const possibleKeys = Object.keys(catalog).filter(k => k.startsWith(cardId));
            if (possibleKeys.length > 0) {
                cardData = catalog[possibleKeys[0]];
            }
        }

        if (!cardData) {
             throw new Error('ไม่พบรหัสการ์ดนี้ใน Master Database');
        }
        ...
        document.getElementById('card-cost').value = cardData.cost !== '-' ? cardData.cost : '0';
        document.getElementById('card-power').value = cardData.power !== '-' ? cardData.power : '0';
        document.getElementById('card-counter').value = cardData.counter !== '-' ? cardData.counter : '0';
        
        document.getElementById('card-attribute').value = cardData.attribute !== '-' ? cardData.attribute : '';
        document.getElementById('card-traits').value = cardData.feature || '';
        document.getElementById('card-effect').value = cardData.effect || '';
        
        document.getElementById('card-set-name').value = 'Thai Official Database'; // Or parse from somewhere

        // Map Color safely
        if(cardData.color) {
            const colorSelect = document.getElementById('card-color');
            const options = Array.from(colorSelect.options).map(o => o.value);
            const primaryColor = cardData.color.split('/')[0].toLowerCase();
            if(options.includes(primaryColor)) {
        ...
        setTimeout(() => {
            if (status.textContent.includes('สำเร็จ')) {
...
function setupAutocomplete() {
    ...
    input.addEventListener('input', async (e) => {
        ...
        if (!cachedCardCatalog) {
            dropdown.innerHTML = `<div class="p-4 text-center text-xs text-gray-500 flex justify-center items-center gap-2"><svg class="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> กำลังโหลดฐานข้อมูลการ์ด...</div>`;
            dropdown.classList.remove('hidden');
            await preloadCardCatalog();
        }

        if (!cachedCardCatalog) {
            dropdown.innerHTML = `<div class="p-3 text-center text-xs text-red-500">โหลดข้อมูลไม่สำเร็จ</div>`;
            return;
        }
